  ...
  var QueryGenerator = {
    ...
    createSchema: function(schema) {
      if (_.isPlainObject(schema)) {
        return [
          'IF NOT EXISTS (SELECT SCHEMA_NAME, CATALOG_NAME',
          'FROM INFORMATION_SCHEMA.SCHEMATA',
          'WHERE SCHEMA_NAME =', wrapSingleQuote(schema.schema),
          'AND CATALOG_NAME =', wrapSingleQuote(schema.database), ')',
          'BEGIN',
            'EXEC sp_executesql N\'CREATE SCHEMA', this.quoteIdentifier(schema.schema),';\'',
          "END;"
        ].join(' ');
      } else {
        return [
    ...
    describeTableQuery: function(tableName, schema, schemaDelimiter, database) {
      var sql = [
        "SELECT",
      ...
      if (database) {
        sql += " AND t.TABLE_CATALOG =" + wrapSingleQuote(database);
      }
    ...
    showTablesQuery: function () {
      return 'SELECT TABLE_NAME, TABLE_SCHEMA, TABLE_CATALOG FROM INFORMATION_SCHEMA.TABLES;';
    },
    ...
    getForeignKeysQuery: function(table, databaseName) {
      ...
      if (table.database) {
        sql += " AND C.TABLE_CATALOG =" + wrapSingleQuote(table.database);
      }
